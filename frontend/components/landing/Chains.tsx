...
export function Chains() {
  ...
  const renderChain = (chain: any) => (
    <motion.div key={chain.name} whileHover={{ scale: 1.05 }} className={`flex items-center gap-2 lg:gap-3 text-white/30 transition-all duration-300 cursor-pointer drop-shadow-none ${chain.hoverTextClass} ${chain.hoverShadowClass} group justify-center`}> 
      <img src={chain.src} alt={`${chain.name} logo`} className="w-6 h-6 lg:w-8 lg:h-8 shrink-0 object-contain grayscale opacity-50 transition-all duration-300 group-hover:grayscale-0 group-hover:opacity-100" /> 
      {chain.textSvg ? (
        chain.textSvg
      ) : (
    ...
    <section id="chains" className="py-24 border-y border-[var(--border-subtle)] bg-[#0F0F1A]/80 backdrop-blur-md relative z-10 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 gap-y-10 gap-x-2 place-items-center sm:grid-cols-3 lg:flex lg:flex-row lg:flex-nowrap lg:items-center lg:justify-center lg:gap-12 xl:gap-20">
          {chains.map((chain) => renderChain(chain))}
        </div>
      </div>
    </section>
  );
