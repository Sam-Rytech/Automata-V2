...
const renderGlowingBrackets = (selected: boolean, color: string) => {
  if (!selected) return null;
  return (
    <>
      <div className="absolute -top-4 -left-4 w-5 h-5 border-t-2 border-l-2 shadow-[-2px_-2px_10px_rgba(233,30,140,0.3)] pointer-events-none z-0 border-[#E91E8C]" style={{ borderColor: color }} />
      <div className="absolute -top-4 -right-4 w-5 h-5 border-t-2 border-r-2 shadow-[2px_-2px_10px_rgba(233,30,140,0.3)] pointer-events-none z-0 border-[#E91E8C]" style={{ borderColor: color }} />
      <div className="absolute -bottom-4 -left-4 w-5 h-5 border-b-2 border-l-2 shadow-[-2px_2px_10px_rgba(233,30,140,0.3)] pointer-events-none z-0 border-[#E91E8C]" style={{ borderColor: color }} />
      <div className="absolute -bottom-4 -right-4 w-5 h-5 border-b-2 border-r-2 shadow-[2px_2px_10px_rgba(233,30,140,0.3)] pointer-events-none z-0 border-[#E91E8C]" style={{ borderColor: color }} />
...
const renderNodeCard = (data: ActionNodeData, selected: boolean, color: string) => {
  return (
    <div className={`relative bg-[#12121A] border p-5 w-[240px] transition-all shadow-xl z-10 ${selected ? 'bg-[#1A1A2E]/80' : 'hover:border-white/20'}`} style={{ borderColor: selected ? color : 'rgba(255,255,255,0.08)' }}>
...
export function ActionNode({ data, selected }: NodeProps<ActionNodeData>) {
  ...
  return (
    <div className="relative">
      {renderGlowingBrackets(selected, color)}
      {renderNodeCard(data, selected, color)}
    </div>
  );
}