...
function OpportunityCard({ opportunity, onDeposit, index }: { opportunity: YieldOpportunity; onDeposit: (opportunity: YieldOpportunity) => void; index: number }) {
  const apy = getAPY(opportunity);
  const protocol = getProtocol(opportunity);
  const chain = getChain(opportunity);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-baseline gap-1">
        ...
        <span className="font-mono text-[8px] text-white/30 uppercase tracking-wider">per year</span>
      </div>
      <div className="flex items-center justify-between">
        <span className="font-mono text-[8px] text-white/30 uppercase tracking-wider">TVL</span>
        <span className="font-mono text-[10px] text-white/60">{formatTVL(opportunity.tvlUsd)}</span>
      </div>
      <button
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {opportunities.slice(0, 5).map((opp, i) => (
          <OpportunityCard key={opp.id || i} opportunity={opp} onDeposit={onDeposit} index={i} />
        ))}
      </div>
    </motion.div>
  );
