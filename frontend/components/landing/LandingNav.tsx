...
const getRedirectUrl = (isMiniPay: boolean, authenticated: boolean) => {
  if (isMiniPay && authenticated) return '/chat';
  const redirect = localStorage.getItem('postLoginRedirect');
  return redirect || '/build';
};

const handleLogin = (login: () => void, isMiniPay: boolean, authenticated: boolean, router: any) => {
  if (authenticated) {
    router.push('/build');
  } else {
...
const handleRedirect = (isMiniPay: boolean, ready: boolean, authenticated: boolean, router: any) => {
...
export function LandingNav() {
  ...
  const handleLaunch = () => {
    ...
    handleLogin(login, isMiniPay, authenticated, router);
  };

  useEffect(() => {
    handleRedirect(isMiniPay, ready, authenticated, router);
