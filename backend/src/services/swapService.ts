...
export async function buildSwapTx(params: any): Promise<any> {
  ...
  if (params.fromChain === 'stellar') {
    ...
    if (!fromAddress || !fromToken || !toToken || !amount) {
      return {
        error:
          'Stellar swap requires fromAddress, fromToken, toToken, and amount.',
      }
    }
  ...
  try {
    ...
    if (!route) {
      return {
        error:
          'Route not found in cache or expired. Please generate a new route first using get_route.',
      }
    }
    ...
    if (!stepTx.transactionRequest) {
      return {
        error: 'LI.FI failed to return a transaction request for this step.',
      }
    }
