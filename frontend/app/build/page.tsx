...
function DraggablePaletteItem({
  ...
  onDrop: (t: ActionType, c: string) => void
  closeDrawer?: () => void
}) {
  ...
  const handleDragEnd = (event: any, info: any) => {
    ...
    if (
      info.point.x >= canvas.left &&
      info.point.x <= canvas.right &&
      info.point.y >= canvas.top &&
      info.point.y <= canvas.bottom
    ) {
    ...
    <div className="relative w-full h-full">
      ...
      <motion.div
        ...
        animate={{
          ...
          zIndex: isRecovering ? -1 : 10,
        }}
        transition={{
          opacity: { duration: isRecovering ? 0 : 1, ease: 'easeInOut' },
        }}
        whileDrag={{
          scale: 1.05,
          zIndex: 99999,
      ...
      >
        ...
        <div className="flex justify-between items-center mb-2 pointer-events-none">
          ...
          <span
            className="text-[10px] opacity-20 group-hover:opacity-100 transition-opacity"
            style={{ color: item.color }}
          >
            ⋮⋮
          </span>
        </div>
        <span className="text-[10px] text-white/50 pointer-events-none">
          {item.desc}
        </span>
      </motion.div>
    </div>
...
function NodeConfigContent({
  ...
  selectedNode: Node
  updateNode: (f: string, v: string) => void
  closeConfig?: () => void
}) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 overflow-y-auto space-y-8 flex-1 custom-scrollbar">
        <div className="bg-[#1A1A2E]/50 border border-[#E91E8C]/20 p-4 flex items-center gap-4 relative">
          ...
          <div>
            <div className="text-[10px] font-bold text-[#E91E8C] tracking-widest uppercase mb-1">
              {selectedNode.type}_MODULE
            </div>
            <div className="text-[8px] text-white/40 tracking-[0.2em] uppercase">
              ID: MOD_{selectedNode.id.split('-')[1]}_XP
            </div>
          </div>
        </div>
        <div className="space-y-4">
          <div className="text-[9px] text-white/60 tracking-[0.3em] uppercase mb-4 border-b border-white/5 pb-2">
            Input Parameters
          </div>
          <div className="space-y-2 relative">
            <div className="flex justify-between items-center">
              <label className="text-[9px] text-white/40 tracking-[0.2em] uppercase">
                Target Chain
              </label>
              <span className="text-[8px] text-[#E91E8C] tracking-widest uppercase">
                [{selectedNode.chain}]
              </span>
            </div>
            <div
              onClick={() =>
                setActiveDropdown(activeDropdown === 'chain' ? null : 'chain')
              }
            ...
            >
              <span className="text-sm font-bold text-white">
                {selectedNode.chain} Network
              </span>
              <ChevronDownIcon
                className={`w-3 h-3 text-white/40 transition-transform ${activeDropdown === 'chain' ? 'rotate-180' : ''}`}
              />
            </div>
            <AnimatePresence>
          ...
          <div className="space-y-2 relative">
            ...
            >
              ...
              <ChevronDownIcon
                className={`w-3 h-3 text-white/40 transition-transform ${activeDropdown === 'asset' ? 'rotate-180' : ''}`}
              />
            </div>
            <AnimatePresence>
        ...
        <div className="space-y-4">
          <div className="text-[9px] text-[#E91E8C] tracking-[0.3em] uppercase mb-2">
            Logic_Override
          </div>
          <div className="bg-[#050508] border border-white/5 p-4 font-mono text-[10px] leading-loose">
            <span className="text-[#E91E8C]">if</span>{' '}
            <span className="text-white">(liquidity_depth &lt; 100k)</span>{' '}
            {'{\n'}
            <span className="text-white/60 pl-4">route = </span>
            <span className="text-[#22C55E]">"BALANCER_V2"</span>
            {';\n'}
            {'}\n'}
            <span className="text-[#E91E8C]">else</span> {'{\n'}
            <span className="text-white/60 pl-4">route = </span>
            <span className="text-[#22C55E]">"UNISWAP_V3"</span>
            {';\n'}
            {'}'}
          </div>
        </div>
      ...
      <div className="p-6 bg-[#0F0F1A] border-t border-white/5 space-y-3 shrink-0">
        <button
          onClick={closeConfig}
          className="w-full bg-[#E91E8C] text-white py-4 font-syne font-bold uppercase text-[10px] tracking-[0.2em] rounded-none hover:bg-[#E91E8C]/80 transition-colors"
        >
          Apply Changes
        </button>
...
function BuildPageContent() {
  ...
  const addNode = (type: ActionType, color: string) => {
    ...
    setNodes((prev) => [
      ...prev,
      { id: newId, type, chain: 'Ethereum', asset: 'ETH', amount: '0', color },
    ])
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#0A0A12] relative">
        <header className="h-16 border-b border-white/5 flex items-center justify-between px-4 sm:px-6 bg-[#0F0F1A] shrink-0 z-20">
          <div className="text-[10px] text-white/40 tracking-[0.3em] uppercase flex items-center gap-4">
            <span className="text-[#E91E8C] font-bold">NETWORK</span>{' '}
          ...
          <div className="flex bg-[#1A1A2E] p-1 border border-white/5">
            ...
            >
              <Squares2X2Icon className="w-3 h-3 inline sm:mr-2" />
            ...
            >
              <CodeBracketIcon className="w-3 h-3 inline sm:mr-2" />
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          ...
          <div
            ref={canvasRef}
            className="flex-1 relative flex flex-col bg-[#0A0A12] z-10 overflow-hidden"
          >
            {view === 'visual' ? (
              <div className="flex-1 bg-dot-grid p-6 sm:p-12 overflow-y-auto custom-scrollbar flex flex-col items-center pb-[150px] lg:pb-12 relative">
                <div className="absolute top-6 left-6 z-30 flex items-center gap-4">
                  <div className="flex items-center gap-4 bg-[#12121A] border border-white/5 p-3">
                    <span className="text-[9px] text-[#E91E8C] tracking-[0.3em] font-bold">
                      DRAFT_V04
                    </span>
                  ...
                  <div className="flex items-center bg-[#12121A] border border-white/5">
                    <button className="p-3 hover:bg-white/5 text-white/40 hover:text-white border-r border-white/5">
                      <MagnifyingGlassPlusIcon className="w-4 h-4" />
                    </button>
                    <button className="p-3 hover:bg-white/5 text-white/40 hover:text-white border-r border-white/5">
                      <MagnifyingGlassMinusIcon className="w-4 h-4" />
                    </button>
                    <button className="p-3 hover:bg-white/5 text-white/40 hover:text-white">
                      <div className="w-4 h-4 border border-current rounded-sm" />
                    </button>
                  </div>
                </div>
                <div className="flex flex-col items-center gap-4 mb-8 mt-16">
                  <div className="w-10 h-10 border border-white/10 flex items-center justify-center font-bold text-[10px] text-white/40">
                    START
                  </div>
                      ...
                      return (
                        ...
                        >
                          <motion.div
                            ...
                            className={`relative transition-all cursor-pointer select-none ${isSelected ? 'p-3 bg-[#1A1A2E]/20' : 'p-0'}`}
                          >
                            {isSelected && (
                              <>
                                ...
                                <div className="absolute -bottom-6 right-0 flex items-center gap-2">
                                  ...
                                  <span className="text-[10px] text-[#22C55E] font-bold tracking-widest">
                                    ~$0.14
                                  </span>
                                </div>
                              </>
                            ...
                            <div
                              className={`relative bg-[#12121A] border p-5 sm:p-6 w-full min-w-[280px] max-w-[320px] shadow-xl ${isSelected ? `border-[${node.color}]` : 'border-white/10 hover:border-white/20'}`}
                            >
                              ...
                              <div className="flex justify-between items-start mb-4 pr-6 relative z-40">
                                <div>
                                  <span className="text-[9px] text-white/40 tracking-[0.2em] uppercase block mb-1">
                                    0{i + 1}_TRIGGER
                                  </span>
                                  <span className="text-sm font-black uppercase tracking-widest block">
                                    {node.type}_ASSET
                                  </span>
                                  <span className="text-[9px] text-white/40 tracking-[0.1em] uppercase">
                                    Source: {node.chain} Network
                                  </span>
                                </div>
                                <button
                                  onClick={(e) => {
                                  ...
                                  className="text-white/10 hover:text-[#EF4444] transition-colors mt-1"
                                >
                          ...
                          {i < nodes.length - 1 && (
                            <div className="w-[1px] h-8 sm:h-12 bg-white/5 my-2 pointer-events-none" />
                          )}
                        </Reorder.Item>
                      )
                    })}
                ...
                {nodes.length === 0 && (
                  <div className="text-white/20 uppercase text-xs tracking-widest mt-10">
                    Canvas Empty
                  </div>
                )}
              </div>
          ...
          <div className="hidden xl:flex w-[380px] bg-[#12121A] border-l border-white/5 flex-col z-20">
            {selectedNode ? (
              <NodeConfigContent
                ...
                closeConfig={() => setSelectedNodeId(null)}
              />
            ) : (
            ...
            <>
              ...
              >
                <NodeConfigContent
                  selectedNode={selectedNode}
                  updateNode={updateSelectedNode}
                  closeConfig={() => setIsMobileConfigOpen(false)}
                />
              </motion.div>
            </>
          )}
        ...
        <footer className="h-16 sm:h-20 bg-[#0F0F1A] border-t border-white/5 flex items-center px-4 sm:px-6 justify-between shrink-0 z-20 relative">
          <div className="flex items-center gap-2 sm:gap-4 font-mono">
            <div className="flex items-center gap-2 sm:gap-3 text-[9px] sm:text-[10px] uppercase tracking-[0.2em]">
              <span className="w-1.5 h-1.5 bg-[#E91E8C] rounded-full" />{' '}
              {nodes.length} Steps
            </div>
          </div>
