...
const ModelLogo = ({
  ...
  <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-3 group cursor-pointer">
    ...
    <span className="hidden sm:block font-syne font-bold text-lg tracking-wider text-white/30 transition-all duration-500 group-hover:text-[${hoverColor}] group-hover:drop-shadow-[0_0_15px_rgba(${hoverColor.replace('#', '').match(/.{1,2}/g).map(x => parseInt(x, 16)).join(',')},0.4)]">
      {text}
    </span>
  </motion.div>
);
    ...
    <section id="models" className="py-12 border-y border-[var(--border-subtle)] bg-[#0F0F1A]/50 backdrop-blur-md relative z-20">
      <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-10">
        ...
        <div className="flex-1 flex flex-wrap items-center justify-center md:justify-end gap-10 sm:gap-16">
          <ModelLogo src="/openai-2.svg" alt="OpenAI" text="GPT" hoverColor="#10A37F" isSvg />
          <ModelLogo src="/claude-logo.svg" alt="Claude" text="CLAUDE" hoverColor="#D97757" />
          <ModelLogo src="/gemini-icon-logo.svg" alt="Gemini" text="GEMINI" hoverColor="#5684D1" />
        </div>
      </div>
    </section>
  );
