...
const ModelLogo = ({
  ...
  <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-3 group cursor-pointer">
    ...
    <span
      className="hidden sm:block font-syne font-bold text-lg tracking-wider text-white/30 transition-all duration-500 group-hover:text-[${color}] group-hover:drop-shadow-[0_0_15px_rgba(${color.replace('#', '').match(/.{1,2}/g).map((hex) => parseInt(hex, 16)).join(',')},0.4)]"
    >
      {text}
    </span>
  </motion.div>
...
export function Models() {
  return (
    <section
      id="models"
      className="py-12 border-y border-[var(--border-subtle)] bg-[#0F0F1A]/50 backdrop-blur-md relative z-20"
    >
