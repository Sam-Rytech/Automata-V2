...
const EXPLORER_URLS: Record<string, (hash: string) => string> = {
  ...
  SOLANA: (hash: string) => `https://explorer.solana.com/tx/${hash}`,
  BASE: (hash: string) => `https://basescan.org/tx/${hash}`,
  POLYGON: (hash: string) => `https://polygonscan.com/tx/${hash}`,
  STACKS: (hash: string) => `https://explorer.hiro.so/txid/${hash}`,
  CELO: (hash: string) => `https://celoscan.io/tx/${hash}`,
  ETHEREUM: (hash: string) => `https://etherscan.io/tx/${hash}`,
};

export function getExplorerUrl(network: string, hash: string): string {
  const normalizedNetwork = network.toUpperCase();
  return EXPLORER_URLS[normalizedNetwork] ? EXPLORER_URLS[normalizedNetwork](hash) : EXPLORER_URLS.ETHEREUM(hash);
}
...
function getTypeColor(tx: Transaction) {
  return TYPE_COLORS[tx.type] || '#E91E8C';
}

function getStatusColor(tx: Transaction) {
  return STATUS_COLORS[tx.status] || '#22C55E';
}
...
export function TransactionCard({ tx, index }: TransactionCardProps) {
  const typeColor = getTypeColor(tx);
  ...
  return (
    ...
    >
      ...
      <div className="flex md:flex-col items-center md:items-end justify-between md:justify-center gap-2 border-t border-white/5 md:border-t-0 pt-4 md:pt-0">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold" style={{ color: statusColor }}>
