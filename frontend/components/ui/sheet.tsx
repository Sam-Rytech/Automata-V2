...
function getSheetContentClassName(side: string) {
  switch (side) {
    ...
    case "left":
      return "inset-y-0 left-0 h-full w-3/4 border-r"
    case "right":
      return "inset-y-0 right-0 h-full w-3/4 border-l"
    case "top":
...
function getSheetContentTransitionClassName(side: string) {
  switch (side) {
    case "bottom":
      return "payload-ending-style:translate-y-[2.5rem] payload-starting-style:translate-y-[2.5rem]"
    case "left":
      return "payload-ending-style:translate-x-[-2.5rem] payload-starting-style:translate-x-[-2.5rem]"
    case "right":
      return "payload-ending-style:translate-x-[2.5rem] payload-starting-style:translate-x-[2.5rem]"
    case "top":
      return "payload-ending-style:translate-y-[-2.5rem] payload-starting-style:translate-y-[-2.5rem]"
...
function SheetContent({
  ...
  const contentClassName = getSheetContentClassName(side)
  const transitionClassName = getSheetContentTransitionClassName(side)
    ...
    <SheetPortal>
      ...
      <SheetPrimitive.Popup
        ...
        className={cn(
          ...
          contentClassName,
          transitionClassName,
          "payload-[side=left]:sm:max-w-sm",
          "payload-[side=right]:sm:max-w-sm",
          className
        )}
        {...props}
      >
