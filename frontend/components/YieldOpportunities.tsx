...
function OpportunityCard({ opportunity, index, onDeposit }: { opportunity: YieldOpportunity; index: number; onDeposit: (opportunity: YieldOpportunity) => void }) {
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {opportunities.slice(0, 5).map((opp, i) => (
          <OpportunityCard key={opp.id || i} opportunity={opp} index={i} onDeposit={onDeposit} />
        ))}
      </div>
    </motion.div>
  );
