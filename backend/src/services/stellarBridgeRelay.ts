...
const MESSAGE_TRANSMITTER_CONTRACT =
  process.env.CCTP_STELLAR_MESSAGE_TRANSMITTER ??
  'CCTP_MESSAGE_TRANSMITTER_PLACEHOLDER'
...
async function pollAttestation(messageHash: string): Promise<string | null> {
  for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
    try {
      ...
      console.log(
        `[Bridge Relay] Waiting for attestation (attempt ${attempt + 1})...`
      )
    } catch (err) {
...
async function submitStellarMint(
  ...
  const relayerAccount = await horizonServer.loadAccount(
  ...
  const operation = contract.call(
    'receive_message',
    nativeToScVal(Buffer.from(message.replace('0x', ''), 'hex'), {
      type: 'bytes',
    }),
    nativeToScVal(Buffer.from(attestation.replace('0x', ''), 'hex'), {
      type: 'bytes',
    })
  )
...
export async function startBridgeRelay(params: {
  ...
  if (!relayerSecret) {
    console.error(
      '[Bridge Relay] STELLAR_RELAYER_SECRET not set — cannot relay mint'
    )
  ...
  ;(async () => {
    try {
      console.log(
        `[Bridge Relay] Starting relay for message ${params.messageHash}`
      )
      ...
      const txHash = await submitStellarMint(
        params.message,
        attestation,
        relayerKeypair
      )
