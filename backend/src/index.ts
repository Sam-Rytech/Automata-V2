...
app.post('/api/bridge/attest', async (req, res) => {
  ...
  try {
    console.log(
      `[Bridge Attest] Polling for ${burnTxHash} on ${sourceChain} → ${destinationChain}`
    )
...
wss.on('connection', (ws) => {
  ...
  ws.send(
    JSON.stringify({ type: 'connected', message: 'Automata WebSocket ready' })
  )
  ...
  ws.on('message', (data) => {
    try {
      ...
      if (message.type === 'monitor' && message.txHash && message.chainId) {
        console.log(
          `[WebSocket] Monitoring: ${message.txHash} on ${message.chainId}`
        )

        pollTransactionStatus(ws, message.txHash, message.chainId).catch(
          (err) => {
        ...
        ws.send(
          JSON.stringify({
            type: 'error',
            message:
              'Unknown message type or missing fields. Expected: { type: "monitor", txHash, chainId }',
          })
        )
      }
    } catch {
