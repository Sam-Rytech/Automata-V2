...
function ChatPageContent() {
    ...
    setTimeout(() => {
      if (executionMode === 'assisted') {
        ...
        const mockPlan: AgentPlan = {
          ...
          totalEstimatedFeeUSD: '$0.47', estimatedTimeSeconds: 42, warnings: []
        };
  ...
  const executePlan = () => {
    ...
    setTimeout(() => {
      setStatus('success');
      const agentMsg: Message = { id: (Date.now() + 1).toString(), role: 'agent', content: 'Sequence completed. 100 USDC has been successfully bridged and swapped. Verification hash: 0x74a...82f' };
  ...
  return (
    <div className="flex h-screen bg-[#0F0F1A] text-white overflow-hidden relative">
      <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="md:hidden absolute top-6 left-6 z-50 p-2 bg-[#1A1A2E] border border-white/10">
            ...
            <div className="space-y-10">
              ...
              {status === 'thinking' && (
                <div className="flex flex-col items-start gap-4">
                  ...
                  <div className="font-mono text-xs text-white/40 space-y-1"><p>{`> Analyzing liquidity pools...`}</p><p>{`> Fetching protocol quotes...`}</p></div>
                </div>
              )}
              <div className="max-w-full sm:max-w-[90%]">
                {status !== 'idle' && <StatusPanel status={status} step={1} totalSteps={2} txHash="0x..." chainId="base" />}
                {status === 'awaiting_approval' && activePlan && <PlanReview plan={activePlan} onApprove={executePlan} onCancel={() => setStatus('idle')} />}
              </div>
            </div>
          </div>
        </div>
        <div className="w-full p-4 sm:p-8 md:p-12 border-t border-white/5 bg-[#0F0F1A]">
          <div className="max-w-4xl mx-auto">
            <div className="bg-[#1A1A2E]/95 border border-white/10 p-1 flex items-center shadow-2xl rounded-none">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Ask Automata..." className="flex-1 bg-transparent border-none outline-none px-4 sm:px-6 font-mono text-xs sm:text-sm text-white placeholder:text-white/20" />
              <button onClick={() => handleSend()} className="bg-[#E91E8C] text-white px-6 sm:px-10 py-3 sm:py-4 font-syne font-bold uppercase text-[10px] sm:text-xs tracking-[0.3em] hover:bg-[#E91E8C]/80 transition-colors rounded-none">Execute</button>
            </div>
          </div>
        </div>
      </main>
