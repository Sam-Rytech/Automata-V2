...
const fetchEvmBalances = async (evmAddress: string): Promise<Partial<Balances>> => {
  try {
...
const fetchStellarBalance = async (stellarAddress: string): Promise<Partial<Balances>> => {
  try {
...
export function useBalances(evmAddress?: string | null, stellarAddress?: string | null) {
  return useQuery({
    ...
    queryFn: async (): Promise<Balances> => {
      ...
      if (evmAddress) {
        Object.assign(results, await fetchEvmBalances(evmAddress));
      }
      if (stellarAddress) {
        Object.assign(results, await fetchStellarBalance(stellarAddress));
      }
      return results;
    },
